...
function adjust_brightness(im_path, factor)
...
def adjust_brightness(im_path, factor):
    im_arr = im2arr(im_path)
...
def adjust_brightness_cv(im_path, factor):
...
file_path = "/home/necromancer/Pictures/searchyfai_dasboard.png"

benchmark_dict = Dict()

j_bench = @benchmark adjust_brightness(file_path, 50)
benchmark_dict["Julia"] = minimum(j_bench.times) / 1e6

py_np_bench = @benchmark $adj_br_np_py(file_path, 50)
benchmark_dict["Python Numpy"] = minimum(py_np_bench.times) / 1e6

py_cv_bench = @benchmark $adj_br_cv_py(file_path, 50)
